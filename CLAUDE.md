...
- **Shared free trial by default**: every user gets a working agent out of the box, funded by a shared key the app operator (currently: the user of this repo) pays for — currently **Gemini**, the durable primary (250,000 TPM — ~29x today's tool-schema payload, so it absorbs a lot of future tool growth without needing re-evaluation). The agent layer supports automatic failover to a second configured trial provider on a transient error (429/5xx) — see `src/server/handleAgentRequest.ts` and §23 in `Roadmap.md` — so either provider having a bad day (Gemini and Groq have already failed independently on the same day in production) doesn't have to take down the whole trial. **Groq is not currently wired in as that second tier**: verified 2026-09-04, every Groq model with tool-calling support was below this app's real tool-schema payload size on Groq's free tier (see `src/server/providers/groq.ts`). §24 (closed 2026-09-04) narrowed the per-request payload to the relevant tool subset for confidently-classified messages (`src/server/classifyIntent.ts`, wired into `buildTools`/`buildSystemPrompt` in `handleAgentRequest.ts`) rather than sending all 17 tools' schemas on every request — real payload for a narrowed bucket is now 17-40% of the old full size, likely enough for most single-intent messages to fit, though the classifier's own fallback path (an unclassified message still gets the full payload) doesn't shrink and needs separate re-verification against Groq's real tokenizer before `TRIAL_FALLBACK_PROVIDER` is flipped back to `groq` in production (see §26 in `Roadmap.md`).
...
- `TRIAL_PROVIDER` / `TRIAL_API_KEY` / `TRIAL_MODEL` env vars configure the shared trial's primary provider; `TRIAL_FALLBACK_PROVIDER` / `TRIAL_FALLBACK_API_KEY` / `TRIAL_FALLBACK_MODEL` configure the optional secondary in the failover chain above (see `.env.example`, `README.md`). `TRIAL_PROVIDER=mock` is a zero-cost stand-in for local development that isn't testing agent reasoning itself.
