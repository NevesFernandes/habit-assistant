...
**v1 tracks Yes/No and, as of §19, Timer.** All four types can be picked when creating a habit. Numeric-value and Checklist still currently store just the label — completion is a single "mark done" toggle for those two, with no number input or interactive checklist yet; building out real tracking for them is still open. Timer habits have a real input mechanism now: a dedicated "timer" tab (`src/components/TimerView.tsx`, state machine in `src/lib/useTimerSession.ts`) with a start/pause/resume/stop stopwatch or countdown, device-local persistence (`src/lib/timerStore.ts`, `localStorage`, no cross-device sync) that survives backgrounding while the app stays alive and reopens paused (never silently fast-forwarding elapsed time) after a full close, and an explicit save-to-habit step once stopped — on top of the pre-existing chat path (`logHabitProgress`), which still works the same for both Numeric and Timer habits.

Habits already build a real tracking/statistics view over time: streak, best streak, completion %, and this-week/month/year/all-time counts are computed (`src/lib/habitStats.ts`) and shown per-habit in `HabitsView` and per-category in `CategoriesView`, plus an overview in a dedicated "stats" dashboard tab (`src/components/Dashboard.tsx`): an overall completion meter + KPI row, a per-category breakdown, and a hand-rolled SVG calendar-heatmap chart (`CalendarHeatmap.tsx`) of one selected habit's daily completion history — no charting library, per the §17 decision (skip a dependency for this small a chart surface). Completion history is logged as habits are completed, but un-completing currently removes that log entry rather than recording the toggle as its own event — so today the log reflects current per-occurrence completion state, not a full history of every completion/uncompletion event.
...
- **Hosting**: **Cloudflare Workers** (static assets + one Worker script), free. (Superseded the earlier "GitHub Pages" idea once it became clear the Anthropic key needed a server-side home — see below. Originally built against Cloudflare Pages specifically; migrated to Workers-with-static-assets on 2026-09-02 once Cloudflare's dashboard stopped offering new Pages projects — same free, on-demand-function shape, just Cloudflare's current product for it. `wrangler.jsonc`'s `assets` field serves the static build; anything that doesn't match a static file falls through to the Worker.)
...
- **Agent / LLM**: called from the same **one small Worker script** (`src/server/worker.ts`, routing `/api/agent`) rather than directly from the browser — calling a provider client-side would put whichever key is in use in the browser bundle/network requests, a real extraction risk even for a private tool. This is a deliberate, narrow amendment to "no backend": it's a single on-demand function, not an always-on server; storage and sync still go straight from the browser to Drive with nothing else in between. See "Cost model / provider strategy" below for which provider and why.
- **Chat turn logic is UI-free (§33)**: everything after the user hits send lives in `ChatSession` (`src/lib/chatEngine.ts`), not in React. That covers calling the agent, carrying out the tool call it chose, the "which one did you mean?" and confirmation follow-ups, and the reply text. `App.tsx` drives it with the real Drive persist, and the headless chat-scenario tests (`npm run test:chat`, `tests/chat/`) drive the same code against real models with JSON fixtures. So new chat behavior belongs in the engine, and gets a scenario.
...
- **Shared free trial by default**: every user gets a working agent out of the box, funded by a shared key the app operator (currently: the user of this repo) pays for — currently **Gemini**, the durable primary (250,000 TPM — ~29x today's tool-schema payload, so it absorbs a lot of future tool growth without needing re-evaluation) and the only active tier in both local dev and production as of 2026-09-08 (kept deliberately identical between the two environments after a same-day incident where they'd drifted — see git history around that date). The agent layer supports automatic failover to a second configured trial provider, then a third last-resort tier, on a transient error (429/5xx) — see `src/server/handleAgentRequest.ts` and §23 in `Roadmap.md` — so either provider having a bad day (Gemini and Groq have already failed independently on the same day in production) doesn't have to take down the whole trial. **Groq is not currently wired in as that second tier**: verified 2026-09-04, every Groq model with tool-calling support was below this app's real tool-schema payload size on Groq's free tier (see `src/server/providers/groq.ts`). §24 (closed 2026-09-04) narrowed the per-request payload to the relevant tool subset for confidently-classified messages (`src/server/classifyIntent.ts`, wired into `buildTools`/`buildSystemPrompt` in `handleAgentRequest.ts`) rather than sending all 17 tools' schemas on every request — real payload for a narrowed bucket is now 17-40% of the old full size, and confirmed live 2026-09-08 to actually fit for classified single-intent messages; the classifier's own fallback path (an unclassified message still gets the full payload) still doesn't shrink, reconfirmed the same day, and still needs that gap closed before `TRIAL_FALLBACK_PROVIDER` is flipped back to `groq` in production (see §26 in `Roadmap.md`). **Cloudflare Workers AI, the third/last-resort tier, was verified end-to-end 2026-09-08** — with both higher tiers forced to fail, it produced a real successful reply — but is not currently enabled in production (needs `WORKERS_AI_ACCOUNT_ID`/`WORKERS_AI_API_TOKEN` as dashboard secrets, a deliberate step, not a default).
- **Bring-your-own-key (BYOK) is a first-class, day-one part of the interface**, not deferred to a "public launch" pass — a Settings panel (`src/components/Settings.tsx`) lets any user plug in their own key for Anthropic, Groq, or Gemini, for unlimited/higher-quality use. BYOK settings (per-provider keys/models, plus the STT-uses-own-key and text-to-speech toggles) are mirrored in browser `localStorage` for synchronous reads but sync across devices as a plain-text `byokSettings` field in the Drive data file (`src/lib/settingsStore.ts`'s `exportState`/`importState`, reconciled only at sign-in — see §30 in Roadmap.md), the same as habits/tasks. This was a deliberate reversal of the original per-device-only design, made after confirming there's no encryption option that wouldn't be either fake (an app-bundled or account-ID-derived "key" is not a secret — same caveat as the §27 debug-log password) or require a user-memorized passphrase never stored anywhere, since this app has no backend account database to derive real protection from. The user explicitly chose plain-text sync over that trade-off, trusting Drive itself as the security boundary.
- **Debug log (§27)**: the same Settings panel has a password-gated "Debug log" section showing the last 50 model-call diagnostics (provider/model, latency, retry count, §24 classify-intent bucket, last user message, reply/error) — built so the user could diagnose the shared-trial cap and the provider-failover chain without needing `wrangler tail` or Cloudflare dashboard access. `VITE_DEBUG_LOG_PASSWORD` is a client-side speed bump only, not real security (fails closed — never unlocks — if unset), and the log itself is device-local (`src/lib/debugLogStore.ts`, `localStorage`), never synced to Drive.
...
- `TRIAL_PROVIDER` / `TRIAL_API_KEY` / `TRIAL_MODEL` env vars configure the shared trial's primary provider; `TRIAL_FALLBACK_PROVIDER` / `TRIAL_FALLBACK_API_KEY` / `TRIAL_FALLBACK_MODEL` configure the optional secondary, and `WORKERS_AI_ACCOUNT_ID` / `WORKERS_AI_API_TOKEN` / `WORKERS_AI_MODEL` the optional third/last-resort tier, in the failover chain above (see `.env.example`, `README.md`). `TRIAL_PROVIDER=mock` is a zero-cost stand-in for local development that isn't testing agent reasoning itself. **`TRIAL_PROVIDER` is committed in `wrangler.jsonc`'s `vars`, not dashboard-only** — a dashboard Runtime variable of the same name gets silently overwritten by the committed value on every rebuild/deploy (a real incident, twice: 2026-09-03 and 2026-09-08). If you ever need production to genuinely run a different primary provider, change it in `wrangler.jsonc` and commit — don't rely on a dashboard-only override.
...
- Audio is recorded in the browser (`MediaRecorder`) and POSTed to the same **small Worker script** (`src/server/worker.ts`, routing `/api/transcribe`), which forwards it to **Groq's Whisper API** and returns text — the browser never holds an STT key, and raw audio is never persisted anywhere (not in Drive, not logged; transcribed and discarded immediately).
