...
- **Auth:** Google Identity Services with scope `drive.file`, used for both sign-in and Drive access. Access tokens last an hour; the app refreshes them when a tap is about to save (so the popup isn't blocked), and on a 401 a Reconnect banner holds the save and replays it (`persist` in `App.tsx`, GitHub issue #7).
